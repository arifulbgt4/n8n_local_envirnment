...
nodes = {n.get('name'): n for n in data['nodes']}

nodes['Quick Intent Router']['parameters']['jsCode'] = r'''const j=$json;
const t=String(j.normalizedQuestion||j.message||'').toLowerCase().trim();
...
const isConfirmation=/^(?:confirm|confirmed|yes|y|জি|জ্বি|হ্যাঁ|হ্যা|ঠিক আছে|ok|okay|কনফার্ম|কনফার্ম করুন)$/i.test(t);
...
else if(state==='AWAITING_CONFIRMATION'&&isConfirmation) intent='CONFIRM_ORDER';
else if(has('order korbo','order korte','অর্ডার করব','অর্ডার করতে','কিনবো','কিনব','buy','purchase','নিতে চাই','নিবো','নেবো','নেব','এইটা নিবো','এটা নিবো')) intent='CREATE_ORDER';
...
nodes['Parse Order Details']['parameters']['jsCode'] = r'''const base=$('Order Context for AI').item.json;
...
const first=(re)=>{for(const s of lines){const m=s.match(re);if(m&&m[1])return m[1].trim();}return '';};
const bnDigits=s=>String(s||'').replace(/[০-৯]/g,d=>'০১২৩৪৫৬৭৮৯'.indexOf(d));

put('customer_name',first(/^(?:নাম|নামটি|customer\s*name|name)\s*[:：-]?\s*(.+)$/i));
const phoneLabel=first(/^(?:মোবাইল|মোবাইল\s*নম্বর|ফোন|ফোন\s*নম্বর|mobile|phone)\s*[:：-]?\s*(.+)$/i);
const phoneAny=bnDigits(msg).match(/(?:\+?88)?01[3-9]\d{8}/)?.[0]||'';
put('phone',bnDigits(phoneLabel)||phoneAny);
put('district',first(/^(?:জেলা|district)\s*[:：-]?\s*(.+)$/i));
put('thana',first(/^(?:থানা|উপজেলা|police\s*station|thana)\s*[:：-]?\s*(.+)$/i));
put('area_name',first(/^(?:এলাকা|এরিয়া|এরিয়া|ইউনিয়ন|ইউনিয়ন|union|area)\s*[:：-]?\s*(.+)$/i));
put('address',first(/^(?:সম্পূর্ণ\s*ঠিকানা|ডেলিভারি\s*ঠিকানা|ঠিকানা|full\s*address|delivery\s*address|address)\s*[:：-]?\s*(.+)$/i));

const inlineValue=(label,nextLabels)=>{
  const re=new RegExp('(?:^|\\s)(?:'+label+')\\s*[:：-]?\\s*(.+?)(?=\\s+(?:'+nextLabels+')\\s*[:：-]?|$)','i');
  const m=msg.match(re);return m?m[1].trim():'';
};
if(!patch.district)put('district',inlineValue('জেলা|district','থানা|উপজেলা|thana|ইউনিয়ন|ইউনিয়ন|এলাকা|area|সম্পূর্ণ\\s*ঠিকানা|ঠিকানা|address'));
if(!patch.thana)put('thana',inlineValue('থানা|উপজেলা|thana','ইউনিয়ন|ইউনিয়ন|এলাকা|area|সম্পূর্ণ\\s*ঠিকানা|ঠিকানা|address'));
if(!patch.area_name)put('area_name',inlineValue('ইউনিয়ন|ইউনিয়ন|এলাকা|এরিয়া|এরিয়া|union|area','সম্পূর্ণ\\s*ঠিকানা|ঠিকানা|address'));

const qty=first(/^(?:পরিমাণ|quantity|qty)\s*[:：-]?\s*(.+)$/i);
if(qty){const n=Number(bnDigits(qty).match(/\d+/)?.[0]||0);if(n>0&&!patch.quantity)patch.quantity=n;}
const pay=first(/^(?:পেমেন্ট|পেমেন্ট\s*মেথড|payment|payment\s*method)\s*[:：-]?\s*(.+)$/i);
...
nodes['Build Order Collection Reply']['parameters']['jsCode'] = r'''const base=$json.base;
const d={...($json.draft||{})};
...
const fields=[['customer_name','নাম'],['phone','মোবাইল'],['district','জেলা'],['thana','থানা'],['area_name','এলাকা/ইউনিয়ন'],['address','সম্পূর্ণ ঠিকানা'],['quantity','পরিমাণ'],['payment_method','পেমেন্ট']];
const missing=fields.filter(([k])=>blank(k));
let reply,state='ORDER_COLLECTING',scheduleFollowup=true;
if(missing.length){
  const labels=Object.fromEntries(fields);
  if(missing.length===1){
    const k=missing[0][0];
    if(k==='address') reply='আর শুধু সম্পূর্ণ ঠিকানাটি দিন। উদাহরণ: সম্পূর্ণ ঠিকানা: তেলিগাতী গাজির পোল';
    else reply=`আর শুধু ${labels[k]} দিন।`;
  }else if(missing.length<=3){
    reply='অর্ডার কনফার্ম করতে আর এই তথ্যগুলো দিন:\n'+missing.map(([k,label])=>`${label}: ________`).join('\n');
  }else{
...
return [{json:{...base,reply,state,scheduleFollowup,contextPatch:{order_draft:d},missingOrderFields:missing.map(([k])=>k)}}];'''

path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n')

check = json.loads(path.read_text())
by_name = {n.get('name'): n for n in check['nodes']}
assert "state==='AWAITING_CONFIRMATION'&&isConfirmation" in by_name['Quick Intent Router']['parameters']['jsCode']
assert "missing.length===1" in by_name['Build Order Collection Reply']['parameters']['jsCode']
print('patched and validated')
